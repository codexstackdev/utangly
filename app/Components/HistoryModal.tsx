...
export const HistoryModal = ({ 
  ...
  // Calculate total collected for the summary
  const totalCollected = history?.reduce((acc, curr) => acc + (curr.amountPaid || 0), 0) || 0;
      ...
      <div className="p-8 max-h-[85vh] flex flex-col">
        ...
        <div className="flex justify-between items-start mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground leading-tight">Detailed collection records</p>
            </div>
          </div>
          <button 
            onClick={onClose} 
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          <div className="p-4 rounded-2xl bg-emerald-500/5 border border-emerald-500/10">
            <p className="text-[10px] font-bold text-emerald-600 uppercase tracking-widest mb-1">Total Collected</p>
            <p className="text-xl font-black text-emerald-600">₱{totalCollected.toFixed(2)}</p>
          </div>
          <div className="p-4 rounded-2xl bg-primary/5 border border-primary/10">
            <p className="text-[10px] font-bold text-primary uppercase tracking-widest mb-1">Transactions</p>
            <p className="text-xl font-black text-primary">{history?.length || 0}</p>
          </div>
        </div>
            ...
            history.map((record, idx) => (
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-bold">{record.payBy || "Anonymous"}</span>
                    </div>
                ...
                <div className="text-right">
                  <p className="text-base font-black text-emerald-600">+₱{(record.amountPaid || 0).toFixed(2)}</p>
                  <p className="text-[9px] font-bold text-muted-foreground uppercase tracking-tighter">Verified</p>
                </div>
              </motion.div>
            ))
          ) : (
